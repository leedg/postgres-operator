...
- *(controller)* **Pooler built-in auth** (T27 ⑤) — `spec.pgbouncer.authSecretRef` 미지정 시 operator 가 PostgresCluster ready primary Pod 에서 `keiailab_pooler_pgbouncer` LOGIN role 자동 생성 (24-byte crypto/rand base64 password) + userlist.txt `<pooler-name>-builtin-auth` Secret 자동 생성 (Pooler OwnerReference). CNPG `cnpg_pooler_pgbouncer` 패턴 호환. Phase=Pending+Reason=BuiltinAuthWaitingForPrimary 로 primary 준비 polling, idempotent ensure. PoolerReconciler.PodExecutor wiring 누락 회귀도 동시 fix (PAUSE/RESUME + SIGHUP reload 회로 복구).
- *(controller)* **Pooler built-in auth password rotation** (T27 ⑥) — annotation `postgres.keiailab.io/rotate-pooler-password=true` 적용 시 `ALTER ROLE` 새 password + userlist.txt Secret in-place update + annotation 제거 + `Pooler.Status.BuiltinAuthLastRotation` timestamp 기록. 신규 unit test `TestPoolerBuiltinAuth_RotatesPasswordOnAnnotation`.
