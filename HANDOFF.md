...
| T29 Pooler TLS auto-issuance | Implementation 80% | cert-manager `Certificate` CR auto-issuance via `spec.pgbouncer.autoTLS` (stage 1 spec + stage 2 controller). **Stage 3 cert-manager kind drill 2026-05-13 → 6/6 PASS** (`hack/smoke-cert-manager.sh`): cert-manager v1.17.4 install + self-signed Issuer + Pooler autoTLS sample → Certificate CR Ready=True → issued Secret → Pooler Deployment mounts. Stages 4 (self-signed fallback) + 5 (rotation observability) remain. |
