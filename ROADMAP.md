...
- [ ] **Automatic failover logic** — new directory `internal/controller/failover/`.
  ...
  - [~] Replica rejoin (`pg_basebackup` or `pg_rewind`) — first-boot `pg_basebackup` + existing-PGDATA old-primary marker generalization + current-primary endpoint main env + `pg_rewind` command-runner + HBA normal-connection auth + fresh `pg_basebackup` fallback all done. Live chaos / rewind drill verification still pending; **PVC + Pod 동시 손실 시나리오는 현재 미지원 — fresh initdb 가 standby promote 를 가린다** ([INC-0001](docs/kb/incident/INC-0001-primary-pvc-loss-no-failover-20260515.md), 2026-05-15 kind smoke evidence).
...
| Date | Change |
|---|---|
| 2026-05-15 | G1 §Failover `Replica rejoin` row 에 PVC+Pod 동시 손실 미지원 명시 + [INC-0001](docs/kb/incident/INC-0001-primary-pvc-loss-no-failover-20260515.md) 링크 — kind smoke chaos drill 에서 발견 (operator promote 미수행, STS 가 fresh initdb primary 재생성, standby timeline 분리). |
