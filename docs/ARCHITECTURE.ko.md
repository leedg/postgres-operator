...
- **소스 버전**: operator `v0.4.0-beta.8` / Helm chart `0.4.0-beta.9`
...
| `ShardSplitJob` | `postgres.keiailab.io/v1alpha1` | Namespaced | 단일 source split workflow; merge와 다중 source 요청은 거부 |
...
| `pkg/version` | ⏳ | 로컬 `version.Combo` 가 더 풍부 — delegation 보류 |
| `pkg/labels` | ✅ | 권장 labels (it28) |
| `pkg/monitoring` | ⏳ | ServiceMonitor 로컬 구현 — delegation 보류 |
| `pkg/networkpolicy` | ⏳ | NetworkPolicy 로컬 구현 — delegation 보류 |
...
| G4 | Online resharding (`ShardSplitJob` 7-step) | 단일 source split 구현; live SLO와 rollback drill 보류 |
...
- 컨테이너 이미지 소스 버전: `ghcr.io/keiailab/postgres-operator:v0.4.0-beta.8` (게시 상태는 별도 추적)
...
- ADR-0025: Repmgr / PgBouncer / Barman 통합 (외부 HA chart parity)
