...
pg-router  (stateless, HPA-scaled)
    ...
    │  - 未来目标: distributed transaction coordinator (2PC + saga; 未实现)
    ├──────┬──────┬──────┬──────
...
### 当前源码(operator v0.4.0-beta.8 / chart 0.4.0-beta.9)

当前 chart 源码包含 **10 个自有 CRD**。已发布 release 和生产验证状态另行跟踪:
...
| `ShardRange` | shard routing metadata 的 source of truth | ⚠️ 控制器已实现，需要 live 验证 |
| `ShardSplitJob` | 单一 source split workflow | ⚠️ 拒绝 merge/多 source，需要 live SLO 与 rollback 演练 |
...
```bash
# 1. Helm chart: 安装 Operator 和 10 个 CRD
# OperatorHub bundle 当前仅含 8 个 CRD，不含 ShardRange/ShardSplitJob
...
**当前源码(operator 0.4.0-beta.8 / chart 0.4.0-beta.9)**:包含 PrometheusRule、Grafana dashboard、ServiceMonitor、WAL归档、备份保留、配置热加载和基于注解的 switchover。发布与 live 验证状态另行跟踪。
...
- Sharding 子系统已有 runtime 代码。`ShardRange`、`pg-router` routing 与单一 source `ShardSplitJob` split 已实现，但拒绝 merge/多 source，live SLO 与自动 rollback 尚未验证/实现。详情见 [`docs/sharding/SHARDING.md`](sharding/SHARDING.md)。
...
- [`ARCHITECTURE.md`](ARCHITECTURE.md) —— 单页架构描述(10 个 CRD 表面 + 自研分布式 SQL + G0-G6 状态 + ADR 交叉链接)
