...
- [ ] **Automatic failover logic** — **(2026-06-16 라이브 RCA: ready standby 가 존재해도 primary force-delete 후 60s 내 standby 승격이 `status.shards[0].primary` 에 미반영. StatefulSet 이 동일 PVC 로 옛 primary 를 ~30s 만에 재생성하여 failover 검출/debounce 윈도우를 preempt — transient pod-delete 는 self-heal 되고 진짜 failover 미발동. ADR-0027 shard-identity ground-up 영역, GA #248. e2e `failover_chaos` Primary-kill Context = PContext Pending.)** new directory `internal/controller/failover/`.
  ...
  - [x] HA election distributed lock (K8s Lease) — `internal/controller/failover/lease.go` (`FailoverLeaseName` + `LeaseConfig` + `NewLease`/`Run`/`IsLeader`, thin adapter over `internal/instance/election.Real` per §2 Simplicity; 2 unit tests with fake clientset verify single-leader + handoff). **`test/e2e/ha_lease_election_test.go`** 신규 작성 (D.2.2): operator manager 2 replica scale → Lease holderIdentity 1 Pod 검증 → leader kill → handoff (LeaseDuration 15s 이내) → failover-lease ↔ manager-lease 분리 검증. `//go:build e2e` PASS. **라이브 e2e PASS (2026-06-16)**: `cmd/main.go` 가 failover lease 를 leader-election-agnostic Runnable 로 배선(전 replica 경쟁) → `test/e2e/ha_lease_election_test.go` 4/4 PASS (holderIdentity 1 Pod + leader Pod kill handoff 15s 이내 + manager-lease 분리). <!-- live-verified: 2026-06-16 -->
...
- [~] **PITR restore** — **(2026-06-16 실측: restore 실행은 미구현 — `pgbackrest restore` 는 실행 중 클러스터에 불가(PG stop + `--delta`/empty PGDATA 필요). 현 sidecar-exec 경로는 stop/recovery orchestration 부재. e2e restore Context = PContext Pending, GA #248. full backup + marker 는 라이브 PASS.)** 구 기록: `BackupRestoreSpec.TargetTime`-driven pgBackRest `restore --type=time --target=...` call path + sidecar exec path both present. **`test/e2e/pitr_restore_e2e_test.go`** 신규 작성 (D.3.2): full backup → marker 'before' + 시점 기록 → 'after' insert → restore type=time targetTime → 'before' 존재 + 'after' 부재 + pg_stat_database checksum_failures=0. `//go:build e2e` 빌드 PASS. 라이브 kind drill 은 cluster mesh 복원 후 별 turn (2026-05-19).
...
- [ ] Sharding plugin interface — **(stale: #124 dead-code 제거로 `internal/plugin/sharding/` 부재 — 2026-06-16 실측 `ls` 부재; 직전 [x]/TestShardingPlugin 무효. routing primitive 는 `internal/router/` 로 이전.)** 구 기록: `internal/plugin/sharding/api.go`. Compile-time interface freeze + `Registry` register/get/Names round-trip + `Capabilities` advertisement + `ErrUnsupported` sentinel guarded by `TestShardingPlugin` umbrella (`internal/plugin/sharding/api_test.go`). RFC 0001~0005 / RFC 0004 (router architecture).
...
- [ ] **2PC / saga** distributed-transaction choice — **(stale: #124 로 `internal/tx/` 전체 제거됨 — 2026-06-16 실측 부재; "8 sub-test PASS" 무효. ADR-0015 결정은 유효하나 코드 미구현 = Phase 4 신규 구현 대상.)** 구 기록: ADR-0015 결정 (2PC primary + saga deferred) + `internal/tx/2pc.go` 실 in-memory state machine 구현: Begin/Enlist/Prepare/Commit/Rollback + State (Active/Prepared/Committed/RolledBack/InDoubt) + parallel goroutine prepare + 부분실패 자동 rollback + InDoubt 표시 + GID/TxID 결정적 발급. 8 sub-test PASS (`TestTwoPhaseCommit`, D.10.2, 2026-05-19). tx log persistence (etcd) + Lease election 통합은 D.2.2 후속.
...
| Date | Change |
|---|---|
| 2026-06-16 | Honesty correction (live audit, session goal): G1 HA election lease `[~]`→`[x]` (failover lease wired in `cmd/main.go` as a leader-election-agnostic runnable; `ha_lease_election` e2e 4/4 PASS live). G3 sharding-plugin / G4 7-step / G5 2PC `[x]`→`[ ]` — the cited `internal/plugin/sharding`, `internal/controller/shardsplit`, `internal/tx` packages were removed in #124 (dead-code removal) and the markers were never corrected (only `internal/router` was revived in #207–#216). PITR restore annotated (restore orchestration unimplemented; e2e Context marked PContext Pending). `failover_chaos` promotion confirmed failing live (`status.shards[0].primary` stays empty 60s; GA #248, ADR-0027 ground-up). |
