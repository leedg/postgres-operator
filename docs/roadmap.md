...
- [ ] **Automatic failover logic** — new `internal/controller/failover/` directory.
  ...
  - [~] Replica rejoin (`pg_basebackup` or `pg_rewind`) — first-boot `pg_basebackup` + existing-PGDATA old-primary marker generalization + current-primary endpoint main env + `pg_rewind` command-runner + HBA normal-connection auth + fresh `pg_basebackup` fallback all done. Live chaos / rewind drill verification still pending; **PVC + Pod 동시 손실 시나리오는 현재 미지원 — fresh initdb 가 standby promote 를 가린다** ([INC-0001](../kb/incident/INC-0001-primary-pvc-loss-no-failover-20260515.md), 2026-05-15 kind smoke evidence).
  ...
  - [x] HA-election distributed lock (K8s Lease) — `cmd/main.go:170-171` `LeaderElection: true` + `LeaderElectionID: "bdce7c33.keiailab.io"`; controller-runtime manages a `coordination.k8s.io/Lease` (only the elected manager runs reconcilers, including failover decisions). Verify: `grep -nE 'LeaderElection(ID)?' cmd/main.go`.
