...
**라우팅 핵심 (능력 사다리)**
- [~] **(E) 프로토콜 종단 — 쿼리 단위 라우팅**: **query-mode 라이브 PG end-to-end 검증 완료 (2026-06-27)** 🎉. 2 trust postgres(shard-0/1) + pg-router `PGROUTER_MODE=query` → psql `SELECT located_on FROM probe WHERE id='alice'` 이 키 추출→murmur3 vindex→샤드 라우팅→실 쿼리 실행→결과 반환까지 동작. **alice→shard-0 / bob→shard-1 / carol→shard-0** 결정적·올바른 분산 확인(bob이 다른 샤드 = 해시 라우팅 실증). 라이브 검증에서 프로토콜 버그(백엔드 핸드셰이크 미소비) 발견·수정. **남은 종단 작업**: ① 일반(비번/scram) 백엔드 인증 대행(현재 trust 한정) ② extended protocol — *부분 완료*: inline-literal Parse + 단일라운드 parameterized(Parse+Bind 연속) 라우팅(`ExtractParamRef`+`bindParams`+Bind 버퍼링). **단, lib/pq는 Parse→Describe→Sync(타입조회)→Bind 순이라 Bind 전 Sync에서 실패** — describe-round 대행(default 샤드로 describe round-trip 후 Bind에서 실샤드 라우팅)이 vtgate급 남은 난제 ③ 멀티샤드 scatter forwarding + 결과 재조립 ④ 백엔드 연결 풀 ⑤ ~~per-query 로그~~ ✅완료. 라이브 검증으로 simple-query 정상·파라미터 한계 확인.
